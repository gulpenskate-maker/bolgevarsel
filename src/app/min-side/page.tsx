...
function showLogin(){
  ...
  ei.onkeydown=e=>e.key==='Enter'&&btn.click()
}
`
  const js2 = `
    ...
    <div style="\${card}">
      ...
      \${locs.length>0?\`
        <div style="display:flex;flex-direction:column;gap:0.6rem">
          <select id="recLoc" style="\${inp}"><option value="">Velg lokasjon</option>\${locs.map(l=>'<option value="'+l.id+'">'+l.name+'</option>').join('')}</select>
...
function renderLocs(){
  ...
  el.innerHTML=locs.map(l=>\`<div style="display:flex;align-items:center;justify-content:space-between;padding:0.75rem 1rem;background:#f8fbfc;border-radius:12px;margin-bottom:0.6rem">
    <div><div style="font-weight:500;color:#0a2a3d">\${l.name}</div><div style="font-size:0.75rem;color:#6b8fa3">\${parseFloat(l.lat).toFixed(4)}°N, \${parseFloat(l.lon).toFixed(4)}°Ø</div></div>
    <button onclick="slettLok('\${l.id}')" style="\${btnD}">🗑 Slett</button></div>\`).join('')
}
...
function renderRecs(){
  ...
  el.innerHTML=recs.map(r=>\`<div id="rec-\${r.id}" style="padding:0.75rem 1rem;background:#f8fbfc;border-radius:12px;margin-bottom:0.6rem">
    <div style="display:flex;align-items:center;justify-content:space-between">
      <div><div style="font-weight:500;color:#0a2a3d">\${r.name||r.phone}</div>
      <div style="font-size:0.75rem;color:#6b8fa3">\${r.name?r.phone+' · ':''}\${locs.find(l=>l.id===r.location_id)?.name||''}
        <span style="background:\${r.active?'#dcfce7':'#f1f5f9'};color:\${r.active?'#16a34a':'#64748b'};padding:1px 7px;border-radius:100px;font-size:0.72rem;margin-left:4px">\${r.active?'Aktiv':'Pauset'}</span></div></div>
...
function setupLocSok(){
  ...
  si.oninput=()=>{
    ...
    locTimer=setTimeout(async()=>{
      ...
      dd.innerHTML=locSugg.map((s,i)=>\`<div data-i="\${i}" style="padding:9px 14px;cursor:pointer;border-bottom:\${i<locSugg.length-1?'1px solid #f0f4f8':'none'};font-size:0.88rem;background:white">
  ...
  si.onblur=()=>setTimeout(()=>dd.style.display='none',200)
  btn.onclick=async()=>{
    if(!locValgt)return
...
`
  ...
  return (
    <div style={{minHeight:'100vh',background:'#e8f4f8',fontFamily:'DM Sans, sans-serif'}}>
      ...
      <script dangerouslySetInnerHTML={{__html: js + js2 + js3}} />
    </div>
  )
}
