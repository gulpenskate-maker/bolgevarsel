...
export async function POST(req: NextRequest) {
  ...
  for (const contact of contacts) {
    const smsMessage = `NODVARSEL fra Bolgevarsel: ${sub.email} har utlost et nodsignal.${location_name ? ` Posisjon: ${location_name}` : ''}${lat && lng ? ` (${lat}, ${lng})` : ''}. Ta kontakt umiddelbart. Ved fare, ring 113.`
    ...
    try {
      ...
      const voiceMessage = `Hei ${contact.name}. Dette er et noedvarsel fra Boelgevarsel. ${sub.email} har utloest et noedsignal.${location_name ? ` Sist kjente posisjon er ${location_name}.` : ''} Vennligst ta kontakt umiddelbart. Ved fare, ring 1 1 3. Gjenta: dette er et noedvarsel.`
