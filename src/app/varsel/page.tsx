...
export default function Varsel() {
  ...
  async function search(q: string) {
    ...
    const res = await fetch(`https://geocoding-api.open-meteo.com/v1/search?name=${encodeURIComponent(q)}&count=6&language=no&format=json`)
  ...
  async function fetchWeather(lat: number, lon: number, name: string) {
    setLoading(true); setError(''); setReport(null); setSuggestions([])
    try {
      ...
      const v = assess(m.current?.wave_height ?? 0, w.current?.wind_speed_10m ?? 0)
      setReport({
        name,
        ...
        seaTemp: m.current?.sea_surface_temperature?.toFixed(1) ?? null,
        wind: (w.current?.wind_speed_10m ?? 0).toFixed(1),
        windDir: degToDir(w.current?.wind_direction_10m ?? 0),
        wave: (m.current?.wave_height ?? 0).toFixed(1),
  ...
  async function submit(e: React.FormEvent) {
    e.preventDefault(); setSuggestions([])
    ...
    if (!hit) { setError('Prøv f.eks: Stavanger, Mandal, Bergen, Ålesund'); setLoading(false); return }
  ...
  const S = (label: string, value: string) => (
    <div style={{background:'#f0f8fc',borderRadius:12,padding:'0.8rem 1rem'}}>
      <div style={{fontSize:'0.68rem',color:'#6b8fa3',textTransform:'uppercase',letterSpacing:'0.06em',marginBottom:'0.25rem'}}>{label}</div>
      <div style={{fontSize:'1.15rem',fontWeight:500,color:'#0a2a3d'}}>{value}</div>
    </div>
  )

  return (
    <div style={{minHeight:'100vh',background:'#e8f4f8',fontFamily:'DM Sans, sans-serif'}}>
      <nav style={{padding:'1.2rem 2rem',borderBottom:'1px solid rgba(10,42,61,0.08)',background:'rgba(232,244,248,0.95)',position:'sticky',top:0,zIndex:100}}>
      ...
      <div style={{maxWidth:560,margin:'0 auto',padding:'3.5rem 1.5rem'}}>
        <div style={{textAlign:'center',marginBottom:'2rem'}}>
          <span style={{fontSize:'0.72rem',fontWeight:500,letterSpacing:'0.12em',textTransform:'uppercase',color:'#4da8cc',display:'block',marginBottom:'0.7rem'}}>Live bølgevarsel</span>
          <h1 style={{fontFamily:'serif',fontSize:'clamp(1.8rem,5vw,2.8rem)',fontWeight:300,color:'#0a2a3d',marginBottom:'0.6rem',letterSpacing:'-0.02em'}}>Sjekk forholdene<br/>ved din lokasjon</h1>
        ...
        <form onSubmit={submit} style={{marginBottom:'0.5rem'}}>
          <div style={{display:'flex',gap:'0.6rem',position:'relative'}}>
            <input
              value={query}
              onChange={e => { setQuery(e.target.value); setReport(null); setError(''); search(e.target.value) }}
            ...
            />
            <button type="submit" disabled={loading} style={{background:'#0a2a3d',color:'white',padding:'1rem 1.4rem',borderRadius:100,border:'none',cursor:'pointer',fontSize:'1rem',fontWeight:500,whiteSpace:'nowrap',minWidth:90}}>
              {loading ? '⏳' : 'Sjekk →'}
            </button>
          </div>
          {suggestions.length > 0 && (
            <div style={{background:'white',borderRadius:16,boxShadow:'0 8px 30px rgba(10,42,61,0.12)',border:'1px solid rgba(10,42,61,0.08)',overflow:'hidden',marginTop:4}}>
              {suggestions.map((s, i) => (
                <div key={i} onClick={() => pick(s)}
                  style={{padding:'0.75rem 1.3rem',cursor:'pointer',borderBottom:i<suggestions.length-1?'1px solid #f0f4f8':'none',background:'white',transition:'background 0.1s'}}
        ...
        {report && (
          <div style={{background:'white',borderRadius:24,padding:'1.6rem',boxShadow:'0 8px 40px rgba(10,42,61,0.1)',border:'1px solid rgba(10,42,61,0.07)',marginTop:'1.5rem'}}>
            ...
            <div style={{fontWeight:600,color:'#0a2a3d',marginBottom:'1rem',fontSize:'1rem'}}>🗺️ {report.name} – {report.date}</div>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'0.6rem',marginBottom:'1rem'}}>
              {S('Lufttemperatur', `🌡️ ${report.temp}°C`)}
              {S('Sjøtemperatur', report.seaTemp ? `🏊 ${report.seaTemp}°C` : '—')}
              {S('Vind', `💨 ${report.wind} m/s fra ${report.windDir}`)}
              {S('Bølger', `🌊 ${report.wave}m · ${report.period}s fra ${report.waveDir}`)}
            </div>
            <div style={{background:report.color+'15',border:`1.5px solid ${report.color}35`,borderRadius:12,padding:'0.85rem 1rem',display:'flex',alignItems:'center',gap:'0.7rem',marginBottom:'1.3rem'}}>
